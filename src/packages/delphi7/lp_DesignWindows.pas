unit lp_DesignWindows;

interface

uses Messages, SysUtils, Classes, Controls, lp_defs, lp_main, lp_form, DesignIntf,
Forms,
  ComponentDesigner;

type
  TDesignWindow = class(TlpForm, IUnknown, IDesignWindow, IDesignNotification,
...
uses Dialogs,{Windows, Clipbrd,} DesignEditors{, VCLEditors};

procedure InitFunc(const Environment : IDesignEnvironment);
begin
//  Environment.
end;
...
procedure Register;
begin
  ComponentDesigner.RegisterInitializeDesigner(InitFunc);
  //ComponentDesigner.Designers.
    RegisterDesignNotification(TDesignWindow.Create(Application));
end;
...
begin
  ...
  ShowMessage('TDesignWindow.Create .');
  Left :=200;
  Top := 30;
  Width := 320;
  Height := 320;
...
//const  ShowCommands: array[TWindowState] of Word =    (SW_SHOWNOACTIVATE, SW_SHOWMINNOACTIVE, SW_SHOWMAXIMIZED);
begin
  if Visible then
...
begin
  ShowMessage('design OOpended#'+Designer.GetRootClassName);
end;
...
procedure TDesignWindow.MsgActivate(var msg: TpgfMessageRec);
begin
  inherited;
  FActive := True;// Msg.Active <> 0;
  if FActive then
    Activated;
end;
