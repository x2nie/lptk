...
type
  ...
  TpgfBaseListBox = class(TpgfWidget)
  private
    FHotTrack: boolean;
  ...
  TpgfTextListBox = class(TpgfBaseListBox)
  protected
    FItems: TStringList;
    ...
    procedure   HandleKeyChar(var AText: word; var shiftstate: TShiftState; var consumed: boolean); override;
  ...
  TlpListBox = class(TpgfTextListBox)
  published
    //x2nie property    AcceptDrops;
    property    Align;
    property    AutoHeight;
    //x2nie property    BackgroundColor default clListBox;
    ...
    property    HotTrack;
    property    Items;
    //x2nie property    ParentShowHint;
    property    PopupFrame;
    //x2nie property    ShowHint;
    property    TabOrder;
    property    Text;
    //x2nie property    TextColor;
    property    OnChange;
    property    OnDoubleClick;
    //x2nie property    OnDragDrop;
    //x2nie property    OnDragEnter;
    //x2nie property    OnDragLeave;
    //x2nie property    OnDragStartDetected;
    property    OnEnter;
    property    OnExit;
    property    OnKeyPress;
    property    OnScroll;
    property    OnSelect;
    //x2nie property    OnShowHint;
  end;
  ...
  public
    ColorName: string;
    ColorValue: TpgfColor;
    constructor Create(const AColorName: string; const AColorValue: TpgfColor);
  end;


  TpgfColorPalette = (cpStandardColors, cpSystemColors, cpWebColors, cpUserDefined);

  
  TpgfBaseColorListBox = class(TpgfBaseListBox)
  private
    FColorBoxWidth: TpgfCoord;
    FColorBoxHeight: TpgfCoord;
    FColorPalette: TpgfColorPalette;
    FShowColorNames: Boolean;
    function    GetColor: TpgfColor;
    procedure   SetColor(const AValue: TpgfColor);
    procedure   SetColorPalette(const AValue: TpgfColorPalette);
  ...
  protected
    ...
    procedure   DrawItem(num: integer; rect: TpgfRect; flags: integer); override;
    procedure   Exchange(Index1, Index2: Integer); override;
//    procedure   HandleKeyChar(var AText: TpgfChar; var shiftstate: TShiftState; var consumed: boolean); override;
    property    Items: TList read FItems;
    property    Color: TpgfColor read GetColor write SetColor;
    property    ColorPalette: TpgfColorPalette read FColorPalette write SetColorPalette default cpStandardColors;
  ...
  TpgfColorListBox = class(TpgfBaseColorListBox)
  published
    //x2nie property    AcceptDrops;
    property    Align;
    property    AutoHeight;
...
function CreateListBox(AOwner: TComponent; x, y, w, h: TpgfCoord): TlpListBox;
...
type
  ...
  TpgfListBoxStrings = class(TStringList)
  protected
    ListBox: TpgfTextListBox;
  public
    constructor Create(AListBox: TpgfTextListBox);
    destructor  Destroy; override;
...
function CreateListBox(AOwner: TComponent; x, y, w, h: TpgfCoord): TlpListBox;
begin
  Result       := TlpListBox.Create(AOwner);
  Result.Left  := x;
  Result.Top   := y;
  Result.Width := w;
  if h > 0 then
...
constructor TpgfListBoxStrings.Create(AListBox: TpgfTextListBox);
begin
  inherited Create;
  ListBox := AListBox;
end;

destructor TpgfListBoxStrings.Destroy;
begin
  ListBox := nil;
  inherited Destroy;
end;

function TpgfListBoxStrings.Add(const s: String): Integer;
begin
...
procedure TpgfListBoxStrings.Delete(Index: Integer);
begin
...
procedure TpgfListBoxStrings.Clear;
begin
  inherited Clear;
  if UpdateCount > 0 then
      Exit;
...
procedure TpgfListBoxStrings.Exchange(Index1, Index2: Integer);
begin
...
procedure TpgfListBoxStrings.Assign(Source: TPersistent);
begin
...
function TpgfBaseListBox.GetFontDesc: string;
begin
  result := FFont.FontDesc;
end;

procedure TpgfBaseListBox.SetFocusItem(const AValue: integer);
var
  old: integer;
begin
...
procedure TpgfBaseListBox.SetFontDesc(const AValue: string);
begin
  FFont.Free;
...
procedure TpgfBaseListBox.SetPopupFrame(const AValue: boolean);
begin
...
procedure TpgfBaseListBox.UpdateScrollbarCoords;
var
  HWidth: integer;
  VHeight: integer;
begin
...
procedure TpgfBaseListBox.SetAutoHeight(const AValue: boolean);
begin
...
procedure TpgfBaseListBox.MsgPaint(var msg: TpgfMessageRec);
begin
...
procedure TpgfBaseListBox.SetFirstItem(item: integer);
begin
  FFirstItem := item;
  UpdateScrollBar;
end;

procedure TpgfBaseListBox.UpdateScrollBar;
var
  pn : integer;
begin
...
procedure TpgfBaseListBox.FollowFocus;
var
  n: integer;
  h: TpgfCoord;
begin
...
function TpgfBaseListBox.ListHeight: TpgfCoord;
begin
  result := height - (2*FMargin);
end;

function TpgfBaseListBox.ScrollBarWidth: TpgfCoord;
begin
  if FScrollBar.Visible then
...
function TpgfBaseListBox.PageLength: integer;
begin
  result := (ListHeight div RowHeight)-1; // component height minus 1 line
end;

procedure TpgfBaseListBox.ScrollBarMove(Sender: TObject; APosition: integer);
begin
...
procedure TpgfBaseListBox.DoChange;
begin
  if Assigned(OnChange) then
    FOnChange(self);
end;

procedure TpgfBaseListBox.DoSelect;
begin
  if Assigned(OnSelect) then
    FOnSelect(self);
end;

procedure TpgfBaseListBox.HandleKeyPress(var keycode: word;
...
procedure TpgfBaseListBox.HandleLMouseDown(x, y: integer; shiftstate: TShiftState);
begin
...
procedure TpgfBaseListBox.HandleLMouseUp(x, y: integer; shiftstate: TShiftState);
begin
...
procedure TpgfBaseListBox.HandleMouseMove(x, y: integer; btnstate: word; shiftstate: TShiftState);
var
  NewFocus: Integer;
  i: Integer;
begin
...
procedure TpgfBaseListBox.HandleMouseScroll(x, y: integer; shiftstate: TShiftState; delta: smallint);
var
  pfi: integer;
begin
  pfi := FFirstItem;
...
procedure TpgfBaseListBox.HandleResize(awidth, aheight: TpgfCoord);
begin
...
procedure TpgfBaseListBox.HandleShow;
begin
...
procedure TpgfBaseListBox.HandlePaint;
var
  n: integer;
  r: TpgfRect;
begin
  ...
  begin
    Canvas.DrawControlFrame(0, 0, r.width, r.height);
    InflateRect(r, -2, -2);
  end;
...
//  r.SetRect(FMargin, FMargin, Width-ScrollBarWidth-(FMargin*2), Height - (FMargin*2));
  ...
  begin
    ...
    begin
      // outer dark border
      Canvas.SetColor(TpgfColor($3b4c71));
      Canvas.SetLineStyle(1, lsSolid);
      Canvas.DrawRect(r);
      InflateRect(r, -1, -1);
      // left top
      Canvas.SetColor(TpgfColor($98b2ed));
      Canvas.DrawLine(r.Left, r.Bottom, r.Left, r.Top);  // left
      Canvas.DrawLine(r.Left, r.Top, r.Right, r.Top);    // top
      // right bottom
      Canvas.SetColor(TpgfColor($4468b8));
      ...
      InflateRect(r, -1, -1);
      ///Canvas.GradientFill(r, TpgfColor($435e9a), TpgfColor($5476c4), gdVertical);
      canvas.SetColor(TpgfColor($435e9a));
      Canvas.DrawRect(r);
      // reset rectangle
      InflateRect(r, 2, 2);
    end;
...
constructor TpgfBaseListBox.Create(AOwner: TComponent);
begin
...
destructor TpgfBaseListBox.Destroy;
begin
  FFont.Free;
  inherited Destroy;
end;

procedure TpgfBaseListBox.BeginUpdate;
begin
  Inc(FUpdateCount);
end;

procedure TpgfBaseListBox.EndUpdate;
begin
  if FUpdateCount = 0 then
    Exit; //==>
  Dec(FUpdateCount);
...
procedure TpgfBaseListBox.Update;
begin
  FFirstItem := -1;
  FFocusItem := -1;
  UpdateScrollBar;
...
function TpgfBaseListBox.ItemCount: integer;
begin
  // This must be overridden in descendant classes!
  result := 17;
end;

function TpgfBaseListBox.RowHeight: integer;
begin
  result := FFont.Height+2;
end;

procedure TpgfBaseListBox.DrawItem(num: integer; rect: TpgfRect; flags: integer);
var
  s: string;
begin
  // This must be overridden in descendant classes!
...
function TpgfTextListBox.GetText: string;
begin
...
procedure TpgfTextListBox.SetText(const AValue: string);
var
  i: integer;
begin
  if AValue = '' then
...
procedure TpgfTextListBox.DrawItem(num: integer; rect: TpgfRect; flags: integer);
begin
  //if num < 0 then
    //Exit; //==>
  pgfStyle.DrawString(Canvas, rect.left+2, rect.top+1, FItems.Strings[num], Enabled);
end;

procedure TpgfTextListBox.Exchange (Index1, Index2: Integer);
begin
  Items.Exchange(Index1, Index2);
end;

procedure TpgfTextListBox.HandleKeyChar(var AText: word;
  var shiftstate: TShiftState; var consumed: boolean);
var
  i: integer;
begin
  ...
  {if (AText > 31) and (AText < 127) {or (Length(AText) > 1 )} then
...
constructor TpgfTextListBox.Create(AOwner: TComponent);
begin
  inherited Create(AOwner);
  FItems := TpgfListBoxStrings.Create(self);
  FFocusItem := -1;
end;

destructor TpgfTextListBox.Destroy;
begin
  TpgfListBoxStrings(FItems).Free;
  inherited Destroy;
end;

function TpgfTextListBox.ItemCount: integer;
begin
  result := FItems.Count;
end;
...
constructor TColorItem.Create (const AColorName: string; const AColorValue: TpgfColor);
begin
  inherited Create;
...
{ TpgfBaseColorListBox }

procedure TpgfBaseColorListBox.SetColorPalette (const AValue: TpgfColorPalette );
begin
...
procedure TpgfBaseColorListBox.SetShowColorNames (const AValue: Boolean );
begin
...
function TpgfBaseColorListBox.GetColor: TpgfColor;
begin
  Result := TColorItem(FItems.Items[FocusItem]).ColorValue;
end;

procedure TpgfBaseColorListBox.SetColor(const AValue: TpgfColor);
var
  i: integer;
begin
...
procedure TpgfBaseColorListBox.SetupColorPalette;
begin
  FreeAndClearColors;
...
procedure TpgfBaseColorListBox.FreeAndClearColors;
var
  i: integer;
begin
  for i := 0 to FItems.Count-1 do
...
procedure TpgfBaseColorListBox.DrawItem(num: integer; rect: TpgfRect; flags: integer);
var
  itm: TColorItem;
begin
  if num < 0 then
  ...
  if FShowColorNames then
    pgfStyle.DrawString(Canvas, FColorboxWidth + 8 + rect.left, rect.top+1, itm.ColorName, Enabled);
end;

procedure TpgfBaseColorListBox.Exchange (Index1, Index2: Integer);
begin
  Items.Exchange(Index1, Index2);
end;

constructor TpgfBaseColorListBox.Create(AOwner: TComponent);
begin
  inherited Create (AOwner );
  FColorBoxWidth := 35;
  FColorBoxHeight := 10;
...
destructor TpgfBaseColorListBox.Destroy;
begin
  FreeAndClearColors;
  FItems.Free;
  inherited Destroy;
...
function TpgfBaseColorListBox.ItemCount: integer;
begin
  result := FItems.Count;
end;
