...
procedure ReadImage_BMP(img : TptkImage; bmp: pointer; bmpsize: longword);

function CreateBMPImage(bmp : pointer; bmpsize: longword) : TptkImage;

implementation

function CreateBMPImage(bmp : pointer; bmpsize: longword) : TptkImage;
begin
  result := TptkImage.Create;
...
var
  ...
  begin
{$ifndef Win32}
    if DisplayDepth = 16 then result := ptkColorToX(cin)
    else
{$endif}
      result := cin;
  end;
