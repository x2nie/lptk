...
uses
  Classes, SysUtils, lptk, ptkwidget, wgScrollBar;

type

  TwgMemo = class(TptkWidget)
  private
    FLines : TStringList;
...
function TwgMemo.CurrentLine: WideString;
begin
      ...
      KEY_LEFT:
              begin  // left
                ...
                begin
                  ...
                  begin
                    // word search...
                    while (FCursorPos > 0) and not ptkIsAlphaNum(copy(CurrentLine,FCursorPos,1))
                      do Dec(FCursorPos);

                    while (FCursorPos > 0) and ptkIsAlphaNum(copy(CurrentLine,FCursorPos,1))
                      do Dec(FCursorPos);
                  end;
      ...
      KEY_RIGHT:
              begin  // right
                ...
                begin
                  ...
                  begin
                    // word search...
                    while (FCursorPos < length(CurrentLine)) and ptkIsAlphaNum(copy(CurrentLine,FCursorPos+1,1))
                      do Inc(FCursorPos);

                    while (FCursorPos < length(CurrentLine)) and not ptkIsAlphaNum(copy(CurrentLine,FCursorPos+1,1))
                      do Inc(FCursorPos);
                  end;
...
function TwgMemo.SelectionText: WideString;
begin
{
...
function TwgMemo.GetText: WideString;
var
  n : integer;
  s : string;
begin
...
procedure TwgMemo.SetText(const AValue : WideString);
var
  n : integer;
  c : string[2];
  s : WideString;
